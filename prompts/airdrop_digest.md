...
- **분자 가드 (v0.11.1, ROI 의미 보강)**: 위 매핑 결과 별점이 ★★★☆☆ 이상이더라도, **분자 가산점 합이 0**(=보상 신호가 단 하나도 없음)이면 별점을 **★★☆☆☆로 cap**. 이유: ROI 정의상 분자=0이면 보상 측정 불가 → "비용만 낮은 활동"이 ★★★ 진입하는 인플레이션 차단. v0.11.0 smoke에서 30/30 항목이 ★★★+가 되어 trigger threshold 의미 무효화된 이슈에 대한 대응. 가드 적용 예: 자본 0 + ≤10분 + 클릭형 만족(분모 4점)이지만 백커·펀딩·임박·리서치·official 모두 미명시 → raw 4점이라 매핑상 ★★★이지만 분자 가드로 ★★☆☆☆ 강등. 분자 신호 단 1개라도 있으면 (예: 펀딩 ≥$10M +1) 매핑 그대로 적용.
