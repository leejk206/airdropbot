# airdrop-pin routine prompt (v0.4.0)
...
> **중요 (snapshot 정규화 룰)**: 답장 대상 본문(=직전 /airdrop output)의 row를 그대로 박제하지 말고 다음 룰로 정규화한다:
>
> 1. `## `, `### `, `**`, `---` 같은 markdown 문법 모두 제거 (Telegram에서 문자 그대로 보임).
> 2. **번호 prefix 제거** — `1. `, `### 3. ` 등 항목 번호는 빼고 헤더는 `<프로젝트명> · ...`으로 시작.
> 3. **"왜 ROI 상위" 필드 제거** — 사용자가 명시적으로 핀한 항목은 ROI 근거가 불필요. `할 일` / `마감` / `출처` 세 필드만 보존 (각 3-space 들여쓰기 + `필드명: 값`).
> 4. 들여쓰기·구분자(`·`)는 `prompts/airdrop_digest.md` §5 ROI Top 10 포맷과 동일.
