...
#### 12.5.7 7차 라이브 실측 (2026-08-02, 978.4s)

`facts=171 anchored=15 targets=10 recipes=18`. `actions.yaml` 18건 중 **9건이 이번 런 재정찰분**이고 나머지 9건은 태그 없는 구 레시피다. 아래 수치는 전부 **재정찰 9건 기준**이다 — 구 레시피를 섞으면 태그 부재(구조적 0)와 모델 판정(실측 0)이 구분되지 않는다.

**(a) 모델은 per-step 판정을 실제로 낸다.** 재정찰 9건 **전부** 스텝 태그를 냈다(9/9). 전건 `false`로 뭉갠 레시피는 **0건**. 스텝 123개 중 `automatable=True`가 **97개(79%)**, `blocker` 사유가 달린 스텝이 26개.

**(b) 실행 상한이 처음으로 0이 아니다.** `ceiling > 0`이 **9/9(100%)**, 중앙값 5, 레시피별 `ceiling/steps` 비율 12~67%.

| 프로젝트 | ceiling/steps | 상한을 끊은 첫 장벽 |
|---|---|---|
| KOR Protocol | 6/9 (67%) | whitelist 등재 필요 — 운영자가 만들 수 없음 |
| AIW3 | 6/14 (43%) | 지갑 서명 프롬프트 확인 |
| Renaiss Protocol | 6/14 (43%) | 실제 자금 입금 |
| MoonPay | 6/17 (35%) | **SMS 일회용 코드 — 사람 소유 전화번호** |
| MetaMask | 5/14 (36%) | 리퍼럴 코드 |
| GRVT | 4/10 (40%) | 지역 제한 확인 |
| Backyard Finance | 4/11 (36%) | 스테이블코인 보유 필요 |
| Polymarket | 4/17 (24%) | CAPTCHA / 봇 챌린지 |
| Abstract | 2/17 (12%) | Passkey/생체 인증 |

**(c) §12.2 대조 — 두 지표는 총합만 우연히 비슷하고, 레시피별로는 8/9에서 갈린다.**

| | 합계 |
|---|---|
| 지갑 기준 prefix (§12.2의 지표) | 39/123 (**32%**) |
| 스텝 태그 기준 prefix | 43/123 (**35%**) |

총합이 가까운 것은 **반대 방향 오차가 상쇄된 결과**지 두 지표가 같아서가 아니다.

- **MoonPay가 §12.2의 3DOS 사례를 그대로 재현했다.** 지갑 스텝이 하나도 없어 지갑 기준으로는 **17/17(100%)** — "전 스텝 실행 안전"이라는 신호가 나온다. 스텝 태그는 **6**이고, 7번째 스텝의 장벽은 *"SMS one-time code requires a human-held phone number"*다. 게이트를 지갑 기준으로 열었다면 실행기가 가짜 전화번호를 채우고 실패했을 것이다. **표현력 문제는 해소됐다.**
- 반대로 **5건은 태그 기준이 지갑 기준보다 크다**(-1~-4). warm 세션에서 `wallet_connect`가 자동 수행 가능하기 때문이다 — 지갑 기준은 그 구간을 **과소평가**하고 있었다. §12.2는 과대평가만 지적했는데, 실측은 양방향 오차를 보여준다.
- **3DOS 자체는 이번에 재정찰되지 않았다**(로테이션 §5.4가 다른 대상을 골랐다). "3DOS가 15가 아니라 2~3으로 나오는가"는 **아직 미검증**이고, MoonPay는 동형 사례이지 그 예측의 확인이 아니다.

**(d) 규칙 6이 더 이상 지배적 거부 사유가 아니다.** 9회 실행 판정: `dry_run` 4 / `rejected` 5, **`pointing_only` 0건**. 6차까지는 전건이 규칙 6에 걸려 포인팅 전용이었다. 거부 5건의 사유는 전부 규칙 1~3(앵커 부재 2 · 무제한 approve 1 · 자본 상한 2)이다 — **안전 규칙은 그대로 물고 있고, 열린 것은 분류 규칙뿐**임이 실측으로 확인됐다.

**(e) 이 측정이 결함 하나를 잡았다.** 규칙 1~5가 early return하면서 `ceiling`을 0으로 눕혀 보고했다(Polymarket 실제 4 → 0). 분포 표본이 자동화 불가능한 쪽으로 편향되므로 `prefilter` 진입 시점에 한 번 계산해 **모든 반환 경로에 싣도록** 고쳤다. 상한은 안전 판정과 독립적인 **측정값**이다.

**(f) 미해결 신호**: `skipped_no_detail_url`이 **9건 재발**했다(6차 0건). 5·6차 대비 `facts`도 193 → 171로 줄었다. 링크 예산 수정(§4.5)의 회귀인지 소스 측 변동인지 **아직 안 봤다.**
