...
### 7차 라이브 (2026-08-02, 978.4s) — 표현력 문제가 해소됐다 (spec §12.5.7)

`facts=171 anchored=15 targets=10 recipes=18`. 18건 중 **9건이 재정찰분**, 나머지는 태그 없는
구 레시피. 아래는 전부 **재정찰 9건 기준** — 섞으면 태그 부재(구조적 0)와 모델 판정(실측 0)이
구분되지 않는다.

| | 6차 | **7차** |
|---|---|---|
| 스텝 태그 생성 | — | **9/9 레시피** (뭉갠 것 0), 스텝 97/123이 `automatable=True` (79%) |
| `ceiling > 0` | — | **9/9 (100%)**, 중앙값 5, 비율 12~67% |
| 게이트 `pointing_only` | **전건** | **0건** — `dry_run` 4 / `rejected` 5 |
| 거부 사유 | 규칙 6 지배 | **규칙 1~3만** (앵커 2·approve 1·자본 2) |

**MoonPay가 §12.2의 3DOS 사례를 그대로 재현했고, 스텝 태그가 그걸 잡았다.** 지갑 스텝이 하나도
없어 지갑 기준으론 **17/17(100%) "전 스텝 안전"**인데, 스텝 태그는 **6**이고 7번째 장벽이
*"SMS 일회용 코드 — 사람 소유 전화번호"*다. 지갑 기준으로 게이트를 열었다면 실행기가 가짜
전화번호를 채웠을 것이다.

**두 지표 총합은 우연히 비슷하다** (지갑 32% / 태그 35%) — 반대 방향 오차가 상쇄된 것이지
같은 게 아니다. 8/9에서 갈리고, 그중 **5건은 태그가 지갑보다 크다** (warm 세션에서
`wallet_connect`가 자동 가능). §12.2는 과대평가만 지적했는데 실측은 **양방향** 오차였다.

**3DOS 자체는 재정찰되지 않았다** (로테이션이 다른 대상 선택). 그 예측은 **미검증**이고
MoonPay는 동형 사례이지 확인이 아니다.

**측정이 결함 하나를 잡았다** — 규칙 1~5가 early return하며 `ceiling`을 0으로 눕혀
보고했다(Polymarket 실제 4 → 0). 표본이 편향되므로 `prefilter` 진입 시 한 번 계산해 **모든
반환 경로에 싣도록** 고쳤다. 내가 쓴 테스트가 키 존재만 보고 prefix 0인 픽스처를 써서
통과해버린 것이 원인. **236 passed**, ruff clean.

### 다음 액션

1. **인계 UX 미구현** (B갈래의 나머지 절반) — `handoff` 응답은 만들었지만 사람에게
   **전달되지 않는다.** broadcast나 알림에 `next_step`·`blocker`를 노출하는 배선이 없다.
   이번 실측으로 실제 인계 지점이 9건 나왔으므로 이제 붙일 대상이 있다.
2. **`skipped_no_detail_url` 9건 재발** (6차 0건) + `facts` 193 → 171. 링크 예산(§4.5)의
   회귀인지 소스 측 변동인지 **아직 안 봤다.**
...
**마지막 갱신**: 2026-08-02 KST (푸터 정정 → §12.3 B갈래 구현 → 7차 라이브 실측).
**마지막 작업자**: ljk9121 (leejk206 GitHub identity)
**현재 HEAD**: `9488797` (master, push 완료) — §12.5 스텝 단위 자동화 태그 + 실행 상한.
직전은 `180f172` (2026-08-01 세션 커밋 7건).

**커밋 완료** (2026-08-02): `9488797` §12.5 스텝 단위 자동화 태그 + 실행 상한 (push 완료).
그 위에 7차 라이브 결과 + guard `ceiling` 편향 수정이 **미커밋**이다:
- `src/airdropbot/execute/guard.py` — `ceiling`을 모든 반환 경로에 실기 (표본 편향 수정)
- `tests/test_guard.py` — 신규 2건 (총 **236 passed**)
- `docs/specs/2026-07-28-*.md` — §12.5.7 실측 기록
- `actions.yaml` — 7차 재정찰 9건이 스텝 태그와 함께 적재됨 (16 → 18 레시피)
- `NEXT.md` — 7차 결과 + 다음 액션

`cache/kb.yaml`은 gitignore. 실행 전 `actions.yaml`·`kb.yaml` 백업은 세션 scratchpad.
